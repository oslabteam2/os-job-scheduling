...
*/
	...
	for(prev = p = head[1]; p != NULL; prev = p, p = p->next){
		...
		if(p->job->wait_time >= 10000 && p->job->curpri < 3){
			p->job->curpri++;
			p->job->wait_time = 0;
			p->job->timer = TimeInterval[p->job->curpri];
...
/*#ifdef DEBUG
	...
	for(p=head[0];p!=NULL;p=p->next){
        ...
        if(current->job->timer > 0 || current->job->curpri > next->job->curpri)
        {
	    	...
	    	if(current->job->curpri > next->job->curpri && current->job->timer == 0)
...
*/
	...
	if(current){
		strcpy(timebuf,ctime(&(current->job->create_time)));
		timebuf[strlen(timebuf)-1]='\0';
		sprintf(string,"current \t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\n",
			current->job->jid,
			current->job->pid,
			current->job->ownerid,
			current->job->run_time,
	...
	for(p=head[0];p!=NULL;p=p->next){
		strcpy(timebuf,ctime(&(p->job->create_time)));
		timebuf[strlen(timebuf)-1]='\0';
		sprintf(string,"The3prioquue\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\n",
			p->job->jid,
			p->job->pid,
			p->job->ownerid,
			p->job->run_time,
		...
		strcpy(timebuf,ctime(&(p->job->create_time)));
		timebuf[strlen(timebuf)-1]='\0';
		sprintf(string,"The2prioquue\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\n",
			p->job->jid,
			p->job->pid,
			p->job->ownerid,
			p->job->run_time,
		...
		strcpy(timebuf,ctime(&(p->job->create_time)));
		timebuf[strlen(timebuf)-1]='\0';
		sprintf(string,"The1prioquue\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\n",
			p->job->jid,
			p->job->pid,
			p->job->ownerid,
			p->job->run_time,
